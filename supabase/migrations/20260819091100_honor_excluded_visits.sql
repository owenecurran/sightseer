-- Applies trip_excluded_visits inside detection. Filtered in the base CTE
-- (and again for manual trips, which read visits directly), so an excluded
-- review affects clustering, the date range and the majority-area label —
-- not just which reviews show.
...
as $$
  with av as (
    ...
    where v.user_id = any(user_ids)
      -- Removed by the user — filtered here, before clustering, so the
      -- trip's dates and label are computed without it too.
      and not exists (
