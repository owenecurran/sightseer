-- The trip's map thumbnail should centre on the DESTINATION, not on the
-- midpoint of its pins. Those differ badly whenever a trip includes a
-- layover: a Seattle trip with one Chicago airport review has a pin
-- centroid somewhere over Montana, so the thumbnail framed empty prairie
-- instead of the city the trip is named after.
--
-- Returning the area place's own coordinates lets the client centre on it
-- directly. Same columns as before plus two, so the old signature has to be
-- dropped first (Postgres won't let create-or-replace change a return type).
